...
# Script to generate command for creating fix-duplicate-zak.sh on server
...
cat << 'SERVER_CMD'
cat > fix-duplicate-zak.sh << 'SCRIPT_EOF'
#!/bin/bash
...
chmod +x fix-duplicate-zak.sh
echo "✅ fix-duplicate-zak.sh created and made executable!"
echo ""
echo "💡 Now run: ./fix-duplicate-zak.sh"
SERVER_CMD
...
echo "   5. Run: ./fix-duplicate-zak.sh"
